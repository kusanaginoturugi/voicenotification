...
- 短い読み上げ中は音楽の音量を下げ、長い読み上げ（80文字以上）は一時停止する。設定で常に一時停止、または常に音量を下げることもできる
...
LINE は画像やスタンプを送ると、本文の代わりに定型文を入れてくる。添付の種類を示すデータ
（`dataMimeType` など）は付いていないので、文字列で判定して言い換える。

| 通知の本文 | 読み上げ |
| --- | --- |
| `yuraが動画を送信しました` | ゆらさんからラインです。動画を送ってきました |
| `スタンプを送信しました` | ゆらさんからラインです。スタンプを送ってきました |
| `243件の新規メッセージ` | 読まない |
| `新しい通知が 1 件あります` | 読まない |
